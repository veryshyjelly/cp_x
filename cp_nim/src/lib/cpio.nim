...
proc nextInt*(r: var Reader): int =
  parseInt(r.nextStr())

proc nextFloat*(r: var Reader): float =
  parseFloat(r.nextStr())
...
proc readBinary*(r: var Reader): Binary =
  ...
  for i, c in s:
    if c == '1':
      ...
      raise newException(ValueError, "Invalid binary char: " & c)
  return Binary(res)
...
# ==============================================================================
# Single value
proc `[]`*[T1](r: var Reader, _: typedesc[T1]): T1 =
  r.read(T1)
...
proc `[]`*[T1, T2, T3](
    r: var Reader, _: typedesc[T1], _: typedesc[T2], _: typedesc[T3]
): (T1, T2, T3) =
  (r.read(T1), r.read(T2), r.read(T3))

proc `[]`*[T1, T2, T3, T4](
    r: var Reader, _: typedesc[T1], _: typedesc[T2], _: typedesc[T3], _: typedesc[T4]
): (T1, T2, T3, T4) =
